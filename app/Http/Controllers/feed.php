...
class feed extends Controller
{
    ...
    function delete(Request $res,$id=0)
    {
        $data = DB::table('post')->where('post_id',$id)->delete();
    ...
    function update(Request $res,$id=0)
    {
        $comment = array('disable' => 1);
        $data = DB::table('post')->where('post_id',$id)->update($comment);
        return redirect('/');
    }

    function update_enable(Request $res,$id=0)
    {
        $comment = array('disable' => 0);
        $data = DB::table('post')->where('post_id',$id)->update($comment);
        return redirect('/');
    }
}
